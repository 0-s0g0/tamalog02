...
const ImageInputModal: React.FC<ImageInputModalProps> = ({
  ...
  const handleImageSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
      ...
      try {
        const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://127.0.0.1:5001/backend';
    ...
    <div className={styles.modalBackground}>
      <div className={styles.modalContent}>
        ...
        <form onSubmit={handleImageSubmit}>
          ...
          {imagePreview && (
            <div className="mb-4">
              <Image src={imagePreview} alt="Preview" width={500} height={300} className="w-full h-auto mb-2" style={{ objectFit: 'contain' }} />
            </div>
          )}
