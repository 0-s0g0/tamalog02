...
const Header2: React.FC<Header2Props> = ({ entries: propsEntries, setEntries: propsSetEntries, fetchData: propsFetchData }) => {
  ...
  // propsから渡された場合はそれを使用
  const finalEntries = propsEntries !== undefined ? propsEntries : entries;
  const finalSetEntries = propsSetEntries !== undefined ? propsSetEntries : setEntries;
   // 認証関連のstate
  ...
  const fetchData = async () => {
    if (propsFetchData) {
      // propsから渡されたfetchDataを使用
      await propsFetchData();
    } else {
      // デフォルトの動作
      if (auth.currentUser) {
        await getEntriesFromFirestore(finalSetEntries);
